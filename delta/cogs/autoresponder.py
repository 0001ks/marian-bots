...
class ar(commands.Cog):
    ...
    async def autoresponder(self, ctx: commands.Context, subcommand: str=None, trigger: str=None, *, response: str=None) -> None:
     if (not ctx.author.guild_permissions.manage_guild):
      emb = discord.Embed(color=discord.Color.yellow(), description=f"{ctx.author.mention} you are missing permissions `manage_server`")
      await ctx.reply(embed=emb, mention_author=False)
      return

     if subcommand is None:
       e = discord.Embed(color=0x2f3136, title="autoresponder", description="sets a autoresponder for your server")
       e.set_author(name=self.bot.user.name, icon_url=self.bot.user.avatar.url)
       e.add_field(name="category", value="config", inline=True)
       e.add_field(name="permissions", value="manage_server", inline=True)
     ...
     if subcommand == "set":
      if trigger is None or response is None:
         e = discord.Embed(color=0xff0000, description=f"{ctx.author.mention} trigger and reponse are required")
         await ctx.reply(embed=e, mention_author=False)
         return 
      ...
      if check is None:
         ...
         embed = discord.Embed(color=0x2f3136, title=f"autoresponder set")
         ...
         e = discord.Embed(color=0x2f3136, title="autoresponder updated")
         res = response.replace("[user_mention]", f"{ctx.author.mention}").replace("[user_tag]", f"{ctx.author}").replace("[user_name]", f"{ctx.author.name}")
      ...
      if trigger is None:
         e = discord.Embed(color=0xff0000, description=f"{ctx.author.mention} trigger is required")
         await ctx.reply(embed=e, mention_author=False)
         return
      ...
      check = collection.find_one({"guildid": ctx.guild.id, "trigger": trigger})
      if check is None:
         embed = discord.Embed(color=discord.Color.yellow(), description=f"there isn't a autoresponder set with this trigger")
         await ctx.reply(embed=embed, mention_author=False)
         return
      elif check is not None:
         collection.delete_one({"guildid": ctx.guild.id, "trigger": trigger})
         e = discord.Embed(color=0x2f3136, title="autoresponder deleted")
         e.add_field(name="trigger", value=trigger)
         await ctx.reply(embed=e, mention_author=False)   

     elif subcommand == "variables":
      embed = discord.Embed(color=0x2f3136, title="autoresponder variables", description=f"[user_mention] - mentions user - {ctx.author.mention}\n[user_tag] - shows user's full name - {ctx.author}\n[user_name] - shows user's name - {ctx.author.name}")
