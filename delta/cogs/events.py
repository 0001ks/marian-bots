...
class events(commands.Cog):
    ...
    async def on_guild_join(self, guild: discord.Guild):
        embed = discord.Embed(color=0x2f3136, description=f"<:x_approve:1038157476882812958> Joined **{guild.name}**, owned by **{guild.owner}** with **{guild.member_count}** members")
        embed.set_footer(text=f"ID: {guild.id}")
        await self.bot.get_channel(1038424086856081408).send(embed=embed)

    @commands.Cog.listener()
    async def on_guild_remove(self, guild: discord.Guild):
       embed = discord.Embed(color=0x2f3136, description=f"<a:zwhitebutterfly:1038394716435259412> Left **{guild.name}**, owned by **{guild.owner}** with **{guild.member_count}** members")
       embed.set_footer(text=f"ID: {guild.id}")
       await self.bot.get_channel(1038424086856081408).send(embed=embed)
