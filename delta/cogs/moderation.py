...
class moderation(commands.Cog):
   ...
   async def ban(self, ctx: commands.Context, member: discord.Member=None, *, reason=None):
    ...
    if member == None:
       e = discord.Embed(color=0x2f3136, title="ban", description="bans member from the server")
    ...
    if member == None or member == ctx.message.author:
        e = discord.Embed(color=0xFFEA00, description=f"> <:warningsaya:1035966642791915581> {ctx.author.mention}: you cannot ban yourself")
        await ctx.reply(embed=e, mention_author=False)
        return
    
    if member.top_role >= ctx.author.top_role and ctx.author.id != ctx.guild.owner.id:
       nope = discord.Embed(color=0xFFEE00, description=f"> <:warningsaya:1035966642791915581> {ctx.author.mention}: you can't ban {member.mention}")
    ...
    if ctx.guild.premium_subscriber_role in member.roles:
     ...
     embed = discord.Embed(color=0x2f3136, description=f"are you sure you want to ban {member.mention}? they are a server booster")
     async def button1_callback(interaction: discord.Interaction):
        ...
        try: 
          ...
          embe = discord.Embed(color=0x2f3136, description=f"> <:banhammer:1035966643882426368> {member.mention} got banned | {reason}")
          await interaction.response.edit_message(embed=embe, view=None)
          try:
           banned = discord.Embed(color=0xFF0000, title="ban case", description=f"you have been banned from {ctx.guild.name}\nIf you want to dispute this situation, contact an admin")
         ...
         no = discord.Embed(color=0xFFEE00, description=f"> <:warningsaya:1035966642791915581> {ctx.author.mention}: i don't have enough permissions to do this")
     ...
     async def button2_callback(interaction: discord.Interaction):
        ...
        embe = discord.Embed(color=0x2f3136, description=f"alright you changed your mind!")
        await interaction.response.edit_message(embed=embe, view=None)
     ...
     try: 
      ...
      embed = discord.Embed(color=0x2f3136, description=f"> <:banhammer:1035966643882426368> {member.mention} got banned | {reason}")
   ...
   async def unban(self, ctx: commands.Context, *, member: discord.User=None):
    ...
    if member == None:
       e = discord.Embed(color=0x2f3136, title="unban", description="unbans member from server")
       e.set_author(name=self.bot.user.name, icon_url=self.bot.user.avatar.url)
       e.add_field(name="category", value="moderation", inline=True)
       e.add_field(name="permissions", value="ban_members", inline=True)
    ...
    try: 
     guild = ctx.guild
     embed = discord.Embed(color=0x2f3136, description=f"{member} has been unbanned")
   ...
   async def kick(self, ctx: commands.Context, member: discord.Member=None, *, reason=None):
    ...
    if member == None:
       e = discord.Embed(color=0x2f3136, title="kick", description="kicks member from server")
       e.set_author(name=self.bot.user.name, icon_url=self.bot.user.avatar.url)
       e.add_field(name="category", value="moderation", inline=True)
    ...
    if member == ctx.author:
     e = discord.Embed(color=discord.Color.yellow(), description=f"> <:warningsaya:1035966642791915581> {ctx.author.mention}: you can't kick yourserlf")
     await ctx.reply(embed=e, mention_author=False)
     return

    if member.top_role >= ctx.author.top_role and ctx.author.id != ctx.guild.owner.id:
       nope = discord.Embed(color=0xFFEE00, description=f"> <:warningsaya:1035966642791915581> {ctx.author.mention}: you can't kick {member.mention}")
       await ctx.reply(embed=nope, mention_author=False)
       return   
    ...
    if ctx.guild.premium_subscriber_role in member.roles:
     button1 = Button(label="yes", style=discord.ButtonStyle.green)
     button2 = Button(label="no", style=discord.ButtonStyle.red)
     embed = discord.Embed(color=0x2f3136, description=f"are you sure you want to kick {member.mention}? they are a server booster")
     async def button1_callback(interaction: discord.Interaction):
        if interaction.user != ctx.author:
            em = discord.Embed(color=0xff0000, description=f"{interaction.user.mention} this is not your message")
            await interaction.response.send_message(embed=em, ephemeral=True)
        ...
        try: 
          ...
          embe = discord.Embed(color=0x2f3136, description=f"> <:banhammer:1035966643882426368> {member.mention} got kicked | {reason}")
          await interaction.response.edit_message(embed=embe, view=None)
          try:
           banned = discord.Embed(color=0xFF0000, title="kick case", description=f"you have been kicked from {ctx.guild.name}\nIf you want to dispute this situation, contact an admin")
           banned.set_thumbnail(url=ctx.guild.icon.url)
           banned.add_field(name="moderator", value=ctx.author)
           banned.add_field(name="reason", value=reason)
           banned.set_footer(text=f"id: {ctx.guild.id}")  
     ...
     try: 
      await member.kick(reason=f"kicked by {ctx.author} - {reason}")
      embed = discord.Embed(color=0x2f3136, description=f"> <:banhammer:1035966643882426368> {member.mention} got kicked | {reason}")
      await ctx.reply(embed=embed, mention_author=False)
      try:
           banned = discord.Embed(color=0xFF0000, title="kick case", description=f"you have been kicked from {ctx.guild.name}\nIf you want to dispute this situation, contact an admin")
           banned.set_thumbnail(url=ctx.guild.icon.url)
