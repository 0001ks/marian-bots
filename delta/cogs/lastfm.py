          ...
          if check[1] == message.content:
            ...
            async with message.channel.typing():
              ...
              if res is not None:   
                ...
                embed = discord.Embed(description=f"<:lastfm:1031802273954811904> You don't have a **Last.fm account** linked. Use `*lf set <username>` to link your **account**.", color=Colors.red)
                await message.reply(embed=embed, mention_author=False)      
      except Exception as e:
          embed = discord.Embed(description=f"{Emojis.wrong} {message.author.mention}: unable to get your recent track - {e}", color=Colors.red)
          await message.reply(embed=embed, mention_author=False)  
...
>   the other commands are displayed in the help command""", inline = False)
    ...
    async def set(self, ctx, ref=None):
            ...
            if not await userhandler.lastfm_user_exists(ref):
                embed = discord.Embed(description=f'<:lastfm:1031802273954811904> **Invalid** Last.fm username',
                                    color=Colors.red)
                ...
                async with self.bot.db.cursor() as cursor:
                    ...
                    embed = discord.Embed(description=f'<:lastfm:1031802273954811904> {ctx.message.author.mention}: Your **Last.fm** username has been set to **{ref}**', color=Colors.red)
                    await ctx.reply(embed=embed, mention_author=False)
                await self.bot.db.commit()  
    ...
    async def remove(self, ctx): 
     async with self.bot.db.cursor() as cursor:
        await cursor.execute("SELECT * FROM lastfm WHERE user_id = {}".format(ctx.author.id))
        check = await cursor.fetchone()
        if check is None: return await ctx.reply(embed=discord.Embed(description=f"<:lastfm:1031802273954811904> You don't have a **Last.fm account** linked. Use `;lf set <username>` to link your **account**.", color=Colors.red), mention_author=False)              
        await cursor.execute("DELETE FROM lastfm WHERE user_id = {}".format(ctx.author.id))
        await self.bot.db.commit()
        await ctx.reply(embed=discord.Embed(color=Colors.green, description=f"{Emojis.check} {ctx.author.mention}: your last fm account has been removed"), mention_author=False)
    
    @lastfm.command(help="view lastfm custom embed variables", description="lastfm")
    @commands.cooldown(1, 3, commands.BucketType.user)
...
"""
        ...
        embed.add_field(name="lastfm variables", value=mes, inline=False)
        await ctx.reply(embed=embed, mention_author=False)

    @lastfm.command(help="view your own lastfm custom embed", description="lastfm", usage="[subcommand]", brief="lastfm embed none - deletes your lastfm custom embed\nlastfm embed view - view the lastfm embed\nany option instead of this will be counted as your embed")
                ...
                async with self.bot.db.cursor() as cursor:
                    ...
                    if starData:
                        ...
                        if cc == "view":
                            await cursor.execute("SELECT mode FROM lfmode WHERE user = ?", (ctx.author.id,))
                            starData = await cursor.fetchone()
                            if starData is None: return await ctx.reply(embed=discord.Embed(colors=Colors.red, description=f"{Emojis.wrong} {ctx.author.mention}: you don't have a lastfm custom embed"), mention_author=False)
                            code = starData[0]
                            embed = discord.Embed(color = 0x2f3136, description = f"```​{code}\n```")
                            return await ctx.reply(embed=embed, mention_author=False)
                        if cc == starData:
                            embed = discord.Embed(description = f"{Emojis.warning} {ctx.message.author.mention}: That is already set as your custom embed mode", color = Colors.yellow)
                            return await ctx.reply(embed=embed, mention_author=False)
                        else:    
                         await cursor.execute("UPDATE lfmode SET mode = ? WHERE user = ?", (cc, ctx.author.id,))
                         await self.bot.db.commit()
                         embed = discord.Embed(description = f"""{Emojis.check} {ctx.message.author.mention}: Updated your custom embed mode to the following:\n``​`\n{cc}``​`""", color =Colors.green)
                        await ctx.reply(embed=embed, mention_author=False)
                    elif starData is None:
                        await cursor.execute("INSERT INTO lfmode VALUES (?, ?)", (cc, ctx.author.id,))
                        await self.bot.db.commit()
                        embed = discord.Embed(color=Colors.green, description = f"""{Emojis.check} {ctx.message.author.mention}: Added your custom embed mode to\n```{cc}```""")
                        await ctx.reply(embed=embed, mention_author=False)
            except Exception as er:
              await ctx.reply(embed = discord.Embed(description=f"<:lastfm:1031802273954811904> {ctx.author.mention}: {er}",color=Colors.red))  
    ...
    async def customcommand(self, ctx, *, cmd=None):
        ...
        if cmd == "none":   
             async with self.bot.db.cursor() as cursor:
                    ...
                    check = await cursor.fetchone()
                    if check is None:
                        return await ctx.reply("you don't have a lastfm custom command")
                    elif check is not None:
                        await cursor.execute(f"DELETE FROM lastfmcc WHERE user_id = {ctx.author.id}")
                    embed = discord.Embed(description=f'<:lastfm:1031802273954811904> {ctx.message.author.mention}: Your **Last.fm** custom command got deleted', color=Colors.red)
        ...
        async with self.bot.db.cursor() as cursor:
                    ...
                    embed = discord.Embed(description=f'<:lastfm:1031802273954811904> {ctx.message.author.mention}: Your **Last.fm** custom command is {cmd}', color=Colors.red)
                    await ctx.reply(embed=embed, mention_author=False)
                    await self.bot.db.commit()  
...
`10` **[{topartist10}]({topartist10url})** {topartist10plays} plays""",color = 0x2F3136)
                ...
                embed = discord.Embed(description=f"<:lastfm:1031802273954811904> You don't have a **Last.fm account** linked. Use `*lfset <username>` to link your **account**.",color=Colors.red)
...
`10` **[{topalbum10}]({topalbum10url})** {topalbum10plays} plays""",color = 0x2F3136)
    ...
    async def whoknows(self, ctx: commands.Context, * ,artist):
     try:   
      ...
      async with ctx.typing():
       ...
       if lis == []:
         await ctx.reply(f"<:lastfm:1031802273954811904> Nobody (not even you) has listened to {artist}") 
         return      
       list = Sort_Tuple(lis)
       listeners = ""
       listenerscount = 0
    ...
    async def nowplaying(self, ctx: commands.Context, *, member: discord.User=None):
        ...
        try:
          ...
          embed = discord.Embed(description=f"{Emojis.wrong} {ctx.author.mention}: unable to get your recent track - {e}", color=Colors.red)
          await ctx.reply(embed=embed, mention_author=False)          
